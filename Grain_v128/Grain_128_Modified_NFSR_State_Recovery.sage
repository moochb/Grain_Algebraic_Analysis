...
#     start = timeit.timeit()
    ...
    for i in range(len(s_out),no_clocks):
        O[i-state_size] =mod(S_out[i-(state_size-2 )]+ S_out[i-(state_size-15 )]+ S_out[i-(state_size-36 )]+ S_out[i-(state_size-45 )]+ S_out[i-(state_size-64 )]+ S_out[i-(state_size-73 )]+ S_out[i-(state_size-89 )]+ S_out[i-(state_size-93 )]+ S_out[i-(state_size-8)]*B_out[i-(state_size-12 )]+ S_out[i-(state_size-13)]*S_out[i-(state_size-20 )]+ B_out[i-(state_size-95)]*S_out[i-(state_size-42 )]+ B_out[i-(state_size-95)]*B_out[i-(state_size-12)]*S_out[i-(state_size-95 )]+ S_out[i-(state_size-60)]*S_out[i-(state_size-79)],2)
        B_out[i] = mod(B_out[i-(state_size-0 )]+ B_out[i-(state_size-26 )]+ B_out[i-(state_size-56 )]+ B_out[i-(state_size-91 )]+ B_out[i-(state_size-96 )]+ B_out[i-(state_size-3)]*B_out[i-(state_size-67 )]+ B_out[i-(state_size-11)]*B_out[i-(state_size-13 )]+ B_out[i-(state_size-17)]*B_out[i-(state_size-18 )]+ B_out[i-(state_size-27)]*B_out[i-(state_size-59 )]+ B_out[i-(state_size-40)]*B_out[i-(state_size-48 )]+ B_out[i-(state_size-61)]*B_out[i-(state_size-65 )]+ B_out[i-(state_size-68)]*B_out[i-(state_size-84)],2)
        S_out[i] = mod((S_out[i-128]+S_out[i-121]+S_out[i-90]+S_out[i-58]+S_out[i-47]+S_out[i-32]),2)
    ...
    for i in range(len(s_out),no_clocks):
        System[i] = S_out[i-(state_size-2 )]+ S_out[i-(state_size-15 )]+ S_out[i-(state_size-36 )]+ S_out[i-(state_size-45 )]+ S_out[i-(state_size-64 )]+ S_out[i-(state_size-73 )]+ S_out[i-(state_size-89 )]+ S_out[i-(state_size-93 )]+ S_out[i-(state_size-8)]*B_out[i-(state_size-12 )]+ S_out[i-(state_size-13)]*S_out[i-(state_size-20 )]+ B[i-(state_size-95)]*S_out[i-(state_size-42 )]+ B[i-(state_size-95)]*B[i-(state_size-12)]*S_out[i-(state_size-95 )]+ S_out[i-(state_size-60)]*S_out[i-(state_size-79)] + O[i-state_size]
#         print(System[i])
    ...
    for j in range(len(System)-244):
        for i in range(128+j,244+j):
