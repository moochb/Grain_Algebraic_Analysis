...
#     start = timeit.timeit()
    ...
    for i in range(state_size,no_clocks):
        O[i-state_size] = mod(S_out[i-(state_size-2)]+S_out[i-(state_size-15)]+S_out[i-(state_size-36)]+S_out[i-(state_size-45)]+S_out[i-(state_size-64)]+S_out[i-(state_size-73)]+S_out[i-(state_size-89)]+S_out[i-(state_size-93)]+B_out[i-(state_size-12)]*S_out[i-(state_size-8)]+S_out[i-(state_size-13)]*S_out[i-(state_size-20)]+B_out[i-(state_size-95)]*S_out[i-(state_size-42)]+S_out[i-(state_size-60)]*S_out[i-(state_size-79)]+B_out[i-(state_size-12)]*B_out[i-(state_size-95)]*S_out[i-(state_size-95)],2)
    ...
    for i in range(len(s_out),no_clocks):
        System[i] = S_out[i-(state_size-2)]+S_out[i-(state_size-15)]+S_out[i-(state_size-36)]+S_out[i-(state_size-45)]+S_out[i-(state_size-64)]+S_out[i-(state_size-73)]+S_out[i-(state_size-89)]+S_out[i-(state_size-93)]+B[i-(state_size-12)]*S_out[i-(state_size-8)]+S_out[i-(state_size-13)]*S_out[i-(state_size-20)]+B[i-(state_size-95)]*S_out[i-(state_size-42)]+S_out[i-(state_size-60)]*S_out[i-(state_size-79)]+B[i-(state_size-12)]*B[i-(state_size-95)]*S_out[i-(state_size-95)]+O[i-state_size]
#         print(System[i])
